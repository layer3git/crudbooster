...
class CBController extends Controller {
  ...
  private function wrapCustomWhere($laravelDbObj) {
	...
	public function getUpdateSingle() {
		...
		$id = Request::get('id');
		$tablePK = CB::pk($table);
		$this->wrapCustomWhere(DB::table($table)->where($tablePK,$id))->update([$column => $value]);
	...
	public function postActionSelected() {
		...
		if($button_name == 'delete') {
			...
			if(CRUDBooster::isColumnExists($this->table,'deleted_at')) {
				$this->wrapCustomWhere(DB::table($this->table)->whereIn($tablePK,$id_selected))->update(['deleted_at'=>date('Y-m-d H:i:s')]);
			}else{
				$this->wrapCustomWhere(DB::table($this->table)->whereIn($tablePK,$id_selected))->delete();
			}
